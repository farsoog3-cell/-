// --- إعدادات شبكة الأونلاين (Socket.io) ---
const SERVER_URL = "https://tank-game-server-o650.onrender.com/";
let socket = null;
let myPlayerId = null;
let myTeam = 'player'; // 'player' (اللاعب الأول - أخضر/أحمر) أو 'enemy' (اللاعب الثاني)
let networkTanks = {}; // تخزين الدبابات المربوطة بالسيرفر
...
function initSocketConnection() {
    ...
    socket.on('connect', () => {
        ...
        showFloatingMsg("تم الاتصال بالسيرفر، في انتظار الخصم...");
    });

    socket.on('assignTeam', (data) => {
        myTeam = data.team; // 'player' or 'enemy'
        showFloatingMsg(myTeam === 'player' ? 'أنت قائد المعسكر الأول 🚩' : 'أنت قائد معسكر العدو ⚔️');
    });

    socket.on('playerJoined', (data) => {
        showFloatingMsg('انضم لاعب جديد إلى المعركة!');
    });

    socket.on('tankMoved', (data) => {
        // تحديث مواقع دبابات اللاعب الآخر عند استلام بيانات من السيرفر
        if (data.playerId !== myPlayerId) {
            let targetList = (data.team === 'player') ? playerTanks : enemyTanks;
            if (targetList[data.tankIndex]) {
                targetList[data.tankIndex].target = new THREE.Vector3(data.x, data.y, data.z);
            }
        }
    });

    socket.on('tankBought', (data) => {
        if (data.playerId !== myPlayerId) {
            let isEnemyTeam = (data.team !== myTeam);
            let color = isEnemyTeam ? (playerFlagType === 'green' ? 0x6b3a2a : 0x2e3b23) : (playerFlagType === 'green' ? 0x2e3b23 : 0x6b3a2a);
            let newTank = createTank(data.x, data.z, color, data.team, data.type);
            
            if (data.team === 'player') playerTanks.push(newTank);
            else enemyTanks.push(newTank);
        }
    });

    socket.on('bulletFired', (data) => {
        if (data.playerId !== myPlayerId) {
            let attackerList = (data.team === 'player') ? playerTanks : enemyTanks;
            let targetList = (data.team === 'player') ? enemyTanks : playerTanks;
            
            let attacker = attackerList[data.attackerIndex];
            let target = targetList[data.targetIndex];
            
            if (attacker && target) {
                if (data.isRocket) fireTacticalMissile(attacker, target, false);
                else fireBullet(attacker, target, false);
            }
        }
    });

    socket.on('disconnect', () => {
        showFloatingMsg("انقطع الاتصال بالسيرفر ⚠️");
    });
}

// --- المتغيرات الأساسية للمشهد واللعبة ---
...
function startGame() {
    ...
    let spawnX = (myTeam === 'player') ? CORNER_OFFSET : -CORNER_OFFSET;
    let spawnZ = (myTeam === 'player') ? CORNER_OFFSET : -CORNER_OFFSET;
    let terrainH = getTerrainHeight(spawnX, spawnZ);

    targetLookAt.set(spawnX, terrainH, spawnZ);
...
function buyPlayerTank(type) {
    ...
    if (playerMoney >= cost) {
        ...
        let spawnCorner = (myTeam === 'player') ? CORNER_OFFSET : -CORNER_OFFSET;
        let pColor = playerFlagType === 'green' ? 0x2e3b23 : 0x6b3a2a;
        let pX = spawnCorner - 50 + (Math.random() - 0.5) * 20;
        let pZ = spawnCorner - 50 + (Math.random() - 0.5) * 20;
        let newTank = createTank(pX, pZ, pColor, myTeam, type);
        ...
        if (myTeam === 'player') playerTanks.push(newTank);
        else enemyTanks.push(newTank);

        // إرسال كود الشراء للسيرفر
        if (socket) {
            socket.emit('buyTank', {
                playerId: myPlayerId,
                team: myTeam,
...
function setupInteraction() {
    ...
    dom.addEventListener('pointerup', (e) => {
        ...
        if (!hasMoved) {
            ...
            let myTanksList = (myTeam === 'player') ? playerTanks : enemyTanks;

            if (selectionMode === 'single') {
                let tankObjects = myTanksList.map(t => t.mesh);
                let intersects = raycaster.intersectObjects(tankObjects, true);
                if (intersects.length > 0) {
                    let hitMesh = intersects[0].object;
                    let found = myTanksList.find(t => t.mesh === hitMesh || t.mesh.children.includes(hitMesh));
            ...
            if (intersects.length > 0) {
                ...
                if (selectionMode === 'all') {
                    myTanksList.forEach((t, index) => {
                        if (t.isDestroyed) return;
                        let offset = new THREE.Vector3((index%3)*8 - 8, 0, Math.floor(index/3)*8);
                        let finalTarget = playerTargetPos.clone().add(offset);
                        t.target = finalTarget;

                        // إرسال كود الحركة عبر الأونلاين
                        if (socket) {
                    ...
                    let index = myTanksList.indexOf(selectedTank);
                    if (socket && index !== -1) {
                        socket.emit('moveTank', {
                            playerId: myPlayerId,
                            team: myTeam,
...
function fireBullet(fromTank, targetTank, emit = true) {
    ...
    if (emit && socket && fromTank.team === myTeam) {
        let attackerList = (fromTank.team === 'player') ? playerTanks : enemyTanks;
        let targetList = (fromTank.team === 'player') ? enemyTanks : playerTanks;
        socket.emit('fireBullet', {
            playerId: myPlayerId,
            team: myTeam,
            attackerIndex: attackerList.indexOf(fromTank),
            targetIndex: targetList.indexOf(targetTank),
            isRocket: false
        });
    }
}

function fireTacticalMissile(fromTank, targetTank, emit = true) {
    const now = Date.now();
...
function updateTanksMovement() {
    ...
    for (let i = tacticalMissiles.length - 1; i >= 0; i--) {
        ...
        if (t >= 1.0) {
            ...
            if (m.fromTeam === myTeam) triggerCameraShake(2.2);
    ...
    allTanksCombined.forEach(tankData => {
        ...
        let opposingList = (tankData.team === 'player') ? enemyTanks : playerTanks;
        enemyTarget = opposingList.find(e => !e.isDestroyed && e.mesh.position.distanceTo(tankData.mesh.position) < (tankData.type === 'rocket' ? 140 : 110));
        ...
        if (tankData.type === 'rocket' && enemyTarget) {
            ...
            fireTacticalMissile(tankData, enemyTarget, true);
        } else {
            if (tankData.target) {
            ...
            if (enemyTarget) fireBullet(tankData, enemyTarget, true);
        }
