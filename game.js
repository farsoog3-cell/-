...
// -------------------------------------------------------------
// التصميم المطور والمفصل بالكامل للدبابات والعربات العسكرية المدرعة
// -------------------------------------------------------------
function createTank(x, z, colorHex, team, type = 'normal') {
    const tankGroup = new THREE.Group();
    let isRocketTank = (type === 'rocket'); // العربة الصاروخية المدرعة
    
    let baseColor = isRocketTank ? (team === 'player' ? 0x1e3a8a : 0x7f1d1d) : colorHex;
    const armorMat = new THREE.MeshStandardMaterial({ color: baseColor, roughness: 0.4, metalness: 0.3 });
    const darkMetalMat = new THREE.MeshStandardMaterial({ color: 0x1e293b, roughness: 0.5, metalness: 0.6 });
    const trackMat = new THREE.MeshStandardMaterial({ color: 0x0f172a, roughness: 0.8 });
    const accentMat = new THREE.MeshStandardMaterial({ color: 0x334155, roughness: 0.3 });
    const glassMat = new THREE.MeshStandardMaterial({ color: 0x38bdf8, roughness: 0.1, transparent: true, opacity: 0.8 });

    if (isRocketTank) {
        // --- تصميم عربة إطلاق الصواريخ المدرعة (المتطورة) ---
        // بدن العربة الرئيسي المصفح
        const body = new THREE.Mesh(new THREE.BoxGeometry(6.5, 2.0, 10.5), armorMat);
        body.position.y = 1.3; body.name = "body"; body.castShadow = true; body.receiveShadow = true; tankGroup.add(body);

        // مقدمة منقار العربة المدرعة
        const frontArmor = new THREE.Mesh(new THREE.BoxGeometry(6.1, 1.2, 3), armorMat);
        frontArmor.position.set(0, 1.2, 5.2);
        frontArmor.rotation.x = -Math.PI / 6;
        frontArmor.castShadow = true; tankGroup.add(frontArmor);

        // مقصورة القيادة الزجاجية المدرعة
        const cabin = new THREE.Mesh(new THREE.BoxGeometry(4.2, 1.3, 3.5), glassMat);
        cabin.position.set(0, 2.5, 1.2);
        cabin.castShadow = true; tankGroup.add(cabin);

        // منصة إطلاق الصواريخ الثقيلة الخلفية
        const launcherBase = new THREE.Mesh(new THREE.BoxGeometry(5.0, 1.0, 5.0), darkMetalMat);
        launcherBase.position.set(0, 2.6, -2.2);
        launcherBase.name = "launcherPod"; launcherBase.castShadow = true; tankGroup.add(launcherBase);

        // حزم صواريخ متعددة الفوهات
        for (let rx of [-1.4, 0, 1.4]) {
            for (let ry of [0.6, 1.5]) {
                const rocketTube = new THREE.Mesh(new THREE.CylinderGeometry(0.45, 0.45, 4.2, 12), accentMat);
                rocketTube.rotation.x = Math.PI / 2;
                rocketTube.position.set(rx, 2.6 + ry, -2.2);
                rocketTube.castShadow = true; tankGroup.add(rocketTube);
            }
        }

        // مسارات جنزير جانبية عريضة للعربة
        [-3.6, 3.6].forEach(tx => {
            const track = new THREE.Mesh(new THREE.BoxGeometry(1.2, 1.6, 11.2), trackMat);
            track.position.set(tx, 0.9, 0);
            track.castShadow = true; tankGroup.add(track);
            
            // عجلات الجنزير البارزة
            for (let wz = -4.5; wz <= 4.5; wz += 3.0) {
                const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.7, 0.7, 1.4, 12), accentMat);
                wheel.rotation.z = Math.PI / 2;
                wheel.position.set(tx > 0 ? tx + 0.2 : tx - 0.2, 0.7, wz);
                wheel.castShadow = true; tankGroup.add(wheel);
            }
        });

    } else {
        // --- تصميم الدبابة الثقيلة التقليدية (المتطورة والواقعية) ---
        // بدن الدبابة السفلي المائل والمضاد للدروع
        const body = new THREE.Mesh(new THREE.BoxGeometry(5.8, 1.8, 9.2), armorMat);
        body.position.y = 1.1; body.name = "body"; body.castShadow = true; body.receiveShadow = true; tankGroup.add(body);

        // درع أمامي مائل إضافي
        const frontArmorSlope = new THREE.Mesh(new THREE.BoxGeometry(5.4, 1.0, 2.5), armorMat);
        frontArmorSlope.position.set(0, 1.1, 4.8);
        frontArmorSlope.rotation.x = -Math.PI / 5;
        frontArmorSlope.castShadow = true; tankGroup.add(frontArmorSlope);

        // برج الدبابة الرئيسي (دائري مصفح)
        const turret = new THREE.Mesh(new THREE.CylinderGeometry(2.8, 3.1, 1.6, 16), armorMat);
        turret.position.y = 2.6; turret.name = "turret"; turret.castShadow = true; tankGroup.add(turret);

        // قبة القائد أعلى البرج
        const commanderCupola = new THREE.Mesh(new THREE.CylinderGeometry(1.1, 1.2, 0.8, 10), darkMetalMat);
        commanderCupola.position.set(0.8, 3.5, -0.6);
        commanderCupola.castShadow = true; tankGroup.add(commanderCupola);

        // رشاش دفاعي فوق القبة
        const machineGun = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 2.2, 6), darkMetalMat);
        machineGun.rotation.x = Math.PI / 2;
        machineGun.position.set(0.8, 4.1, 0.2);
        tankGroup.add(machineGun);

        // سبطانة المدفع الرئيسي الطويلة والمزودة بمدملق حراري
        const cannon = new THREE.Mesh(new THREE.CylinderGeometry(0.32, 0.4, 7.5, 12), darkMetalMat);
        cannon.rotation.x = Math.PI / 2;
        cannon.position.set(0, 2.6, 4.8); cannon.name = "cannon"; cannon.castShadow = true; tankGroup.add(cannon);

        // فوهة المدفع الكبيرة
        const muzzle = new THREE.Mesh(new THREE.CylinderGeometry(0.45, 0.45, 1.2, 12), armorMat);
        muzzle.rotation.x = Math.PI / 2;
        muzzle.position.set(0, 2.6, 8.0);
        tankGroup.add(muzzle);

        // مسارات الجنزير الجانبية الثقيلة مع العجلات
        [-3.2, 3.2].forEach(tx => {
            const track = new THREE.Mesh(new THREE.BoxGeometry(1.1, 1.5, 9.8), trackMat);
            track.position.set(tx, 0.8, 0);
            track.castShadow = true; tankGroup.add(track);

            for (let wz = -3.8; wz <= 3.8; wz += 2.5) {
                const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.65, 0.65, 1.3, 12), accentMat);
                wheel.rotation.z = Math.PI / 2;
                wheel.position.set(tx > 0 ? tx + 0.15 : tx - 0.15, 0.65, wz);
                wheel.castShadow = true; tankGroup.add(wheel);
            }
        });
    }
...
function buyPlayerTank(type) {
    ...
    if (playerMoney >= cost) {
        ...
        showFloatingMsg(type === 'rocket' ? 'تم طلب عربة صواريخ مدرعة' : 'تم طلب دبابة ثقيلة');
    }
}
            ...
            if (selectionMode === 'single') {
                ...
                if (intersects.length > 0) {
                    ...
                    if (found && !found.isDestroyed) {
                        selectedTank = found;
                        showFloatingMsg('تم تحديد الوحدة العسكرية');
                        return;
                    }
                }
            }
