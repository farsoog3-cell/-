const soundFiles = {
    ...
    // الاتصال الفوري بالسيرفر عبر مكتبة Socket.io
    const socket = io("https://tank-game-server-o650.onrender.com");

    socket.on('connect', () => {
        console.log("تم الاتصال بسيرفر اللعبة بنجاح! المعرف:", socket.id);
    });
    ...
    // استقبال بيانات الدبابات الأخرى وتحديثها
    let remotePlayersTanks = {};
    socket.on('updateTanks', (allPlayers) => {
        // يمكنك هنا مطابقة بيانات الدبابات القادمة من السيرفر مع الدبابات الموجودة
        // console.log("تحديثات السيرفر:", allPlayers);
    });
    ...
    function updateTanksMovement() {
        ...
        allTanksCombined.forEach(tankData => {
            ...
            // إرسال موقع الدبابة الحالية للسيرفر إذا كانت تابعة للاعب
            if (tankData.team === 'player' && isMoving) {
                socket.emit('tankMove', {
                    id: socket.id,
                    x: tankData.mesh.position.x,
                    y: tankData.mesh.position.z,
                    angle: tankData.mesh.rotation.y
                });
            }
